...
if(isset($_POST['delete'])){
	$info = getData();
	$delete_query = "DELETE FROM `adds` WHERE id = '$info[0]'";
	try{
...
header('window.location: delete.php');
			...
			echo"<script>  
alert('deleted data not available!');
header('window.location: delete.php');
</script>";
		}
	}
	}catch(Exception $ex){
		echo("errorin deleted".$ex->getMessage());
	}
}
//search
...
<div>
	  <input type="submit"name="delete"value="Delete user"style="background-color:#803399;color:white;font-weight: bold;font-size: 20px;" >
</div>
